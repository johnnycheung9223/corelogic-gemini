// CoreLogic AI Board — Worker v16.0
// 三董事架構：Perplexity（情報）→ DeepSeek（審計）→ Azure OpenAI GPT-4o（策略）
// 串行合作：每個 AI 都能看到前一個的回應
// POST / with {"issue": "..."} → returns {perplexity, deepseek, strategy}
...
async function callPerplexity(issue, apiKey) {
  const body = {
    ...
    messages: [
      {
        role: "system",
        content: "你係 CoreLogic AI 技術情報董事。專門收集 BLE IMU、MediaPipe、Sensor Fusion、下肢康復科技最新技術資訊同業界動態。提供具體、可執行的技術建議。用繁體中文回應，格式清晰。"
      },
      { role: "user", content: `CoreLogic AI Sprint 1 議題：${issue}\n\n請提供：1)最新技術情報 2)可落地方案 3)具體工具/庫推薦` }
    ]
  };
...
async function callAzureOpenAI(issue, perplexityReport, deepseekReport, endpoint, apiKey) {
  const url = `${endpoint}chat/completions?api-version=2024-10-21`;
  const body = {
    model: "gpt-4o",
    messages: [
      {
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      return Response.json({ status: "ok", version: "16.0.0", board: ["perplexity", "deepseek", "azure-gpt4o"] }, { headers: CORS });
    }
    ...
    try {
      ...
      // Step 3: 策略董事（整合兩份報告）— 如果冇 Azure key 就跳過
      ...
      if (env.AZURE_OPENAI_KEY && env.AZURE_OPENAI_ENDPOINT) {
        strategyText = await callAzureOpenAI(
          ...
          env.AZURE_OPENAI_ENDPOINT, env.AZURE_OPENAI_KEY
